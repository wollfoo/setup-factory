...
This guide explains how to install dependencies for Factory Code skills.
...
pip install -r .factory/skills/ai-multimodal/scripts/requirements.txt
...
```bash
cd .factory/skills/ai-multimodal/scripts
...
**ai-multimodal** (`.factory/skills/ai-multimodal/scripts/requirements.txt`):
...
```bash
cd .factory/skills/{skill-name}/scripts
...
2. **`.factory/skills/{skill-name}/.env`** (skill-specific config)
3. **`.factory/skills/.env`** (shared skills config)
4. **`.factory/.env`** (global Factory config)
...
```bash
chmod +x .factory/skills/*/scripts/*.py
```
...
# Run all tests
pytest .factory/skills/*/scripts/tests/ -v

# Check coverage across all skills
pytest .factory/skills/*/scripts/tests/ --cov=.factory/skills --cov-report=html
```
...
5. Open an issue: https://github.com/anthropics/factory-code/issues
