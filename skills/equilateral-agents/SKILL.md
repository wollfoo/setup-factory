...
- Khi cần security review đa agent cho repo hiện tại.
- Khi muốn thiết lập code quality gate trước khi merge hoặc trước khi release.
- Khi cần validate deployment và kiểm tra hạ tầng (infrastructure) trước khi deploy production.
- Khi muốn droid orchestration nhiều subagent thực tế trong thư mục `agents/` để tổng hợp báo cáo.

## Inputs

- Repo hiện tại đã checkout và build được (các lệnh build/test chạy được từ shell).
- Test suite sẵn sàng (ví dụ `npm test`, `pytest`, `go test`, v.v.).
- Các agents/subagents tương ứng đã được cấu hình và khả dụng trong môi trường (xem phần **Agents Directory** ở cuối file).
- Kết nối đến bất kỳ dịch vụ external/CI/CD nào mà workflow cần sử dụng (nếu có).

## Success criteria

- Sinh được báo cáo tổng hợp (security review, code quality, deploy validation, infrastructure check) theo các mẫu trong phần **Workflows Sử Dụng Subagents Thực Tế** và **Evidence-Based Reporting**.
- Không còn critical issue chưa được xử lý trong báo cáo trước khi deploy/merge.
- Các workflow `/ea:*` chạy thành công hoặc dừng với hướng dẫn khắc phục rõ ràng.
- Có audit trail đầy đủ (reports, agent logs, performance metrics) giống cấu trúc trong phần **File Structure**.
